---
title: "Pedicción de Actividad Judicial mediante algoritmos de Machine Learning"
...
En el caso particular del Poder Judicial de Entre Ríos, en su área de Planificación y Estadística, interesa abordar el problema de manera progresiva, buscando como punto de partida modelos que nos permitan generar proyecciones confiables (con bajo margen de error) e interpretables por nuestros usuarios. La interpretabilidad es una parte importante de la estadística pública judicial pues trata particularmente del enfoque de los productos hacia un destinatario no técnico.    

En tal sentido, empleando información disponible hemos creado un dataset de trabajo para analizar y proyectar las presentaciones que puede tener el Poder Judicial, tanto a nivel global como para sus distintas unidades geográficas (circunscripciones judiciales) u operativas (órganos).   
...
df_presentanciones_xmes %>%   
  plot_time_series(fecha, presentaciones_abogados, 
                   .title = "Presentaciones digitales de abogados STJER", 
                   .x_lab = "mes", 
...
## Causas Iniciadas, presentaciones digitales y actos procesales por mes 
...
## Causas Iniciadas, presentaciones digitales y actos procesales por día 
...
Los resultados de las predicciones de los modelos muestran: 
- Modelo 4, basado en una Regresión Lineal, presenta los mejores resultados considerando los errores de los modelos (residuos). Considerando el error porcentual absoluto medio (MAPE) el modelo tiene una alta calificación (Lewis:1982), con un error promedio del **10.5%**. Este modelo plantea una regresión de las presentaciones a partir de la tendencia (*as.numeric(date)*) y la estacionalidad (*month(date)*).
...
Modelos univariantes vs multivariantes

El análisis de las presentaciones digitales considerando su evolución temporal junto a la evolución de otros indicadores judiciales representa el análisis de un proceso estocástico multivariante.

El fundamento de la predictibilidad de un evento está en que su proudcción se asocia a factores que podrían permanecer estables más allá del período relevado (muestral).

Series estacionarias (compuesta por unidades menores repetidas con propiedad estadísticas semejantes unas de otras: específicamente su 'autocovarianza') como aquellas compatibles con la hipótesis de haber sido generada por procesos estacionarios.
